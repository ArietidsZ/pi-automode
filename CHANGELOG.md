...
- **Classifier output budgets** — Send each classifier stage the output room its context-fit check already reserves: the answer allowance plus the reasoning budget. On providers where hidden reasoning shares the output ceiling, reasoning no longer consumes the decision tool call room. After a length stop, the detailed retry now uses the largest context-safe ceiling instead of repeating the identical request. (#42)
