...
- **Classifier output budgets** — Send each classifier stage the amount its context-fit check already reserves: the answer allowance plus the #51 reasoning reserve. This raises the shared output ceiling. It does not keep hidden reasoning out of the decision tool call. After a length stop, the detailed retry uses the model output limit or the estimated context room, whichever is smaller, and never goes below the first ceiling. (#42)
